import { Question, Country } from "../../types/country"
...
const questionGenerators: QuestionGenerator[] = [
  {
    ...
    generate: (country: Country, countries: Country[]): Question | null => {
      ...
      const options = shuffleArray([
        ...shuffleArray(otherCapitals).slice(0, 3),
        country.capital[0],
      ])
    ...
    generate: (country: Country, countries: Country[]): Question | null => {
      ...
      return {
        ...
        options: shuffleArray([
          ...otherFlags.map((c) => c.name.common),
          country.name.common,
        ]),
      }
    },
  },
  {
    ...
    generate: (country: Country): Question | null => {
      ...
      const variations = [
        Math.round(population * 0.5),
        Math.round(population * 1.5),
...
export function generateQuestions(
  ...
  const validCountries = countries.filter(
    (country) => country.capital?.[0] && country.flags && country.population
  )
